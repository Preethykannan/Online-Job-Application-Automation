...
def is_logged_in_LN() -> bool:
    ...
    Returns:
        True if logged in (landed on feed page), False otherwise.
    '''
    try:
        # Check if current page is the LinkedIn feed (post-login)
        ...
        # Check for typical login-related elements
        if try_linkText(driver, "Sign in"): return False
        if try_xp(driver, '//button[@type="submit" and contains(text(), "Sign in")]'): return False
        if try_linkText(driver, "Join now"): return False

    except Exception as e:
        print_lg("Login check error:", e)

    # If none of the above matched, assume user is logged in
    print_lg("✅ Didn't find 'Sign in' or 'Join now'. Assuming login was successful.")
    return True


def login_LN() -> None:
    '''
    Open LinkedIn login page and wait for user to login manually.
    '''
    driver.get("https://www.linkedin.com/login")
    print_lg("🔐 Please login to LinkedIn manually in the opened Chrome window...")
    

    try:
        # Wait for user to land on the feed page (indicates login success)
        wait.until(EC.url_to_be("https://www.linkedin.com/feed/"))
        print_lg("✅ Login successful!")
    except Exception as e:
        print_lg("❌ Login failed or timed out. Please try again.")
        pyautogui.alert("Login failed. Please retry manually in the browser window.", "Login Error")
        raise e

while not is_logged_in_LN():
    login_LN()
    ...
    for Question in all_questions:
        ...
        if text: 
            ...
            if not prev_answer or overwrite_previous_answers:
                ...
                elif ('hear' in label or 'come across' in label) and 'this' in label and ('job' in label or 'position' in label): answer = "https://github.com/niyas-softdev"
...
def main() -> None:
    try:
        ...
        if not os.path.exists(default_resume_path):
            pyautogui.alert(
                text=f'❗ Your resume "{default_resume_path}" is missing!',
        ...
        driver.get("https://www.linkedin.com/login")
        print_lg("🔐 Please login manually in the opened Chrome window...")

        # ✅ Wait up to 180s (3 minutes) for user to login manually
        login_timeout = 180
        for i in range(login_timeout):
            if is_logged_in_LN():
                print_lg("✅ Detected successful LinkedIn login.")
                break
            sleep(1)
        else:
            pyautogui.alert("❌ Login failed or timed out. Please try again.", "Login Error")
            return
        ...
        print_lg("🎯 Starting job application run...")
        driver.switch_to.window(linkedIn_tab)
        ...
        while run_non_stop:
            if cycle_date_posted:
                ...
                global date_posted
                date_posted = date_options[
                    0 if date_posted == date_options[-1]
            ...
            if dailyEasyApplyLimitReached:
                print_lg("🚫 Daily Easy Apply limit reached. Stopping run.")
                break

    except NoSuchWindowException:
        print_lg("❌ Chrome window was closed manually. Exiting gracefully...")
        ...
        try:
            ...
            print_lg(f"External job links collected:   {external_jobs_count}")
            print_lg("                              ----------")
            print_lg(f"Total applied or collected:     {easy_applied_count + external_jobs_count}")
            print_lg(f"Failed jobs:                    {failed_count}")
            print_lg(f"Irrelevant jobs skipped:        {skip_count}")

            if randomly_answered_questions:
                print_lg("\nQuestions answered randomly (fallback):\n" + "\n".join(str(q) for q in randomly_answered_questions))

            quote = choice([
                "You're one step closer than before.",
                "Keep going. Your job is out there.",
                "Progress is progress, no matter how small."
            ])
            pyautogui.alert(f"\n{quote}", "Exiting..")
            print_lg(quote, "Closing the browser...")

            if tabs_count >= 10:
                msg = "⚠️ You have more than 10 tabs opened. Please close or bookmark them!"
            ...
            print_lg("Error in cleanup/final logging.", e)
